...
class IrisAgent:
    ...
    def pose_cb(self, msg):
        ...
        if self.origin_x is None and self.state.connected and getattr(self, 'pos_initialized', False):
    ...
    def check_guardrails(self):
        ...
        if dist_origin > 10.0 and self.state.mode == "OFFBOARD":
            rospy.logerr_throttle(2, f"Guardrail limit exceeded: Distance {dist_origin:.1f}m > 10m! RTL")
        ...
        while not rospy.is_shutdown():
            ...
            if self.agent_state == "TAKEOFF":
                if alt > 2.0:
                    ...
                    rospy.loginfo("Publishing setpoints to enable OFFBOARD mode...")
                    for i in range(20):
                        cmd = TwistStamped()
                        cmd.header.stamp = rospy.Time.now()
                        self.vel_pub.publish(cmd)
                ...
                if detected:
                    ...
                    vx = 0.8 * math.sin(self.search_t * 0.5)
                    vy = 0.8 * math.cos(self.search_t * 0.3)
                ...
                if dist < 0.2:
                    ...
                    # dx_body = dx * cos(yaw) + dy * sin(yaw)
                    # dy_body = -dx * sin(yaw) + dy * cos(yaw)
